##################################################
#Made by Adventquest                             #
#Initialize haste                                #
##################################################

scoreboard objectives add HAS_DATA dummy
scoreboard players set @s HAS_DATA 0

scoreboard objectives add HAS_TOT dummy
scoreboard players set @s HAS_TOT 0

scoreboard objectives add HAS_BASE dummy
scoreboard players set @s HAS_BASE 0

scoreboard objectives add HAS_EQ dummy
scoreboard players set @s HAS_EQ 0

scoreboard objectives add HAS_SP dummy
scoreboard players set @s HAS_SP 0

scoreboard objectives add HAS_PO dummy
scoreboard players set @s HAS_PO 0

scoreboard objectives add HAS_EH dummy
scoreboard players set @s HAS_EH 0

scoreboard objectives add HAS_EXT dummy
scoreboard players set @s HAS_EXT 0

scoreboard objectives add TIMER_HAS_SP dummy
scoreboard objectives add TIMER_HAS_EQ dummy
scoreboard objectives add TIMER_HAS_EXT dummy
scoreboard objectives add TIMER_HAS_PO dummy
scoreboard objectives add TIMER_HAS_EH dummy

scoreboard objectives add HAS_UPGRADE_REQ dummy
scoreboard players set @s HAS_UPGRADE_REQ 1
