...
execute if score temp_damage CAL matches 1.. as @e[distance=..5,type=!player,nbt={HurtTime:10s},tag=!AMRE,limit=1] at @s on attacker if entity @s[advancements={att2_test:enchantment/armorbreak=true}] run tag @e[type=!player,distance=..0,limit=1] add TEMP_ATK
