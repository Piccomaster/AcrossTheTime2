...
##dialog tip
tellraw @a[distance=..128] [{translate:att2.dailyquest.ryliath.name},{translate:att2.dailyquest.kortaek.3.name}]
...
##remove reputation
scoreboard players operation heros REPUTATION -= kortaek_dailyquest_3_fail_reputation DAILYQUEST

##remove chronotons
execute if score TOTAL BANK >= kortaek_dailyquest_3_fail_chronoton DAILYQUEST run return run scoreboard players operation TOTAL BANK -= kortaek_dailyquest_3_fail_chronoton DAILYQUEST
scoreboard players operation @s CHRONOTON -= kortaek_dailyquest_3_fail_chronoton DAILYQUEST
