#####################################
#Made by Adventquest                #
#Obtain item splash_potion_56          #
#####################################

give @s minecraft:splash_potion[custom_name='potion56.name',minecraft:custom_data={EquipmentType:'potion',Rarity:'unc',Shop:'buy',Potion:'56'},lore={translate:'potion56.name'},lore=[{translate:'potion56.lore.1'},{translate:'potion56.lore.2'},{translate:'potion56.stat'}],max_stack_size=4,potion_contents={custom_color:5740891,custom_effects:[{amplifier:2,duration:0,id:'instant_health',show_particles:0b}]}] 1