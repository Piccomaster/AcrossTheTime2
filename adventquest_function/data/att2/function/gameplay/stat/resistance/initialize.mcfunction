##################################################
#Made by Adventquest                             #
#Initialize resistance                           #
##################################################

scoreboard objectives add RES_DETECTION minecraft.custom:minecraft.damage_resisted

scoreboard objectives add RES dummy
scoreboard players set @s RES 0

scoreboard objectives add RES_DATA dummy
scoreboard players set @s RES_DATA 0

scoreboard objectives add RES_TOT dummy
scoreboard players set @s RES_TOT 0

scoreboard objectives add RES_BASE dummy
scoreboard players set @s RES_BASE 0

scoreboard objectives add RES_EQ dummy
scoreboard players set @s RES_EQ 0

scoreboard objectives add RES_SP dummy
scoreboard players set @s RES_SP 0

scoreboard objectives add RES_PO dummy
scoreboard players set @s RES_PO 0

scoreboard objectives add RES_EH dummy
scoreboard players set @s RES_EH 0

scoreboard objectives add RES_EXT dummy
scoreboard players set @s RES_EXT 0

scoreboard objectives add TIMER_RES_SP dummy
scoreboard objectives add TIMER_RES_EQ dummy
scoreboard objectives add TIMER_RES_EXT dummy
scoreboard objectives add TIMER_RES_PO dummy
scoreboard objectives add TIMER_RES_EH dummy
scoreboard objectives add POTION_CHECK_RES dummy

scoreboard objectives add RES_UPGRADE_REQ dummy
scoreboard players set @s RES_UPGRADE_REQ 4