##################################################
#Made by Adventquest                             #
#Initialize dahäl regeneration                   #
##################################################

scoreboard objectives add DAR dummy
scoreboard players set @s DAR 0

scoreboard objectives add DAR_DATA dummy
scoreboard players set @s DAR_DATA 0

scoreboard objectives add DAR_TOT dummy
scoreboard players set @s DAR_TOT 0

scoreboard objectives add DAR_BASE dummy
scoreboard players set @s DAR_BASE 0

scoreboard objectives add DAR_EQ dummy
scoreboard players set @s DAR_EQ 0

scoreboard objectives add DAR_SP dummy
scoreboard players set @s DAR_SP 0

scoreboard objectives add DAR_PO dummy
scoreboard players set @s DAR_PO 0

scoreboard objectives add DAR_EH dummy
scoreboard players set @s DAR_EH 0

scoreboard objectives add DAR_EXT dummy
scoreboard players set @s DAR_EXT 0

scoreboard objectives add DAR_LE dummy
scoreboard players set @s DAR_LE 0

scoreboard objectives add TIMER_DAR_SP dummy
scoreboard objectives add TIMER_DAR_EQ dummy
scoreboard objectives add TIMER_DAR_EXT dummy
scoreboard objectives add TIMER_DAR_PO dummy
scoreboard objectives add TIMER_DAR_EH dummy
scoreboard objectives add TIMER_DAR_LE dummy
scoreboard objectives add POTION_CHECK_DAR dummy

scoreboard objectives add DAR_UPGRADE_REQ dummy
scoreboard players set @s DAR_UPGRADE_REQ 1