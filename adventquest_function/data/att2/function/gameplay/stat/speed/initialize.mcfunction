##################################################
#Made by Adventquest                             #
#Initialize strength                             #
##################################################

scoreboard objectives add SPD dummy
scoreboard players set @s SPD 0

scoreboard objectives add SPD_DATA dummy
scoreboard players set @s SPD_DATA 0

scoreboard objectives add SPD_TOT dummy
scoreboard players set @s SPD_TOT 0

scoreboard objectives add SPD_BASE dummy
scoreboard players set @s SPD_BASE 0

scoreboard objectives add SPD_EQ dummy
scoreboard players set @s SPD_EQ 0

scoreboard objectives add SPD_SP dummy
scoreboard players set @s SPD_SP 0

scoreboard objectives add SPD_PO dummy
scoreboard players set @s SPD_PO 0

scoreboard objectives add SPD_EH dummy
scoreboard players set @s SPD_EH 0

scoreboard objectives add SPD_EXT dummy
scoreboard players set @s SPD_EXT 0

scoreboard objectives add TIMER_SPD_SP dummy
scoreboard objectives add TIMER_SPD_EQ dummy
scoreboard objectives add TIMER_SPD_EXT dummy
scoreboard objectives add TIMER_SPD_PO dummy
scoreboard objectives add TIMER_SPD_EH dummy
scoreboard objectives add POTION_CHECK_SPD dummy

scoreboard objectives add SPD_UPGRADE_REQ dummy
scoreboard players set @s SPD_UPGRADE_REQ 2