...
execute as @e[distance=..10,type=interaction,tag=EnderChest,nbt={interaction:{}}] at @s on target if entity @s[advancements={att2_test:ender_chest/deposit=true}] run tag @n[distance=..0,type=interaction,tag=EnderChest,nbt={interaction:{}}] add TempSelect

##clear nearly interaction data
execute as @e[distance=..10,type=interaction,tag=EnderChest] run data remove entity @s interaction

##revoke test
advancement revoke @s only att2_test:ender_chest/deposit
...
##Cooling down
execute if score #cooldown CAL matches 1.. if score @s ENDERCHEST matches 1 run return run tellraw @s [{translate:att2.ender_chest.deposit.chronotons.confirm,with:[{score:{name:"#Chronotons",objective:CAL},color:gold}]}]
##Cooling complete
execute unless score #cooldown CAL matches 1.. if score @s ENDERCHEST matches 1 run return run tellraw @s [{translate:att2.ender_chest.deposit.confirm}]
