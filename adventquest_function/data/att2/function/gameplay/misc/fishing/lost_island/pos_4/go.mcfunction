...
#random POS select
execute if score tic TIMECOUNTER matches 2 as @n[tag=FISH_SPOT,type=item_display,distance=..20,tag=SPOT_SELECT] at @s run function att2:gameplay/misc/fishing/lost_island/pos_4/pos_select
#set area score
execute if score tic TIMECOUNTER matches 2 if entity @a[distance=..30] as @n[tag=FISH_SPOT,type=item_display,distance=..20] at @s run function att2:gameplay/misc/fishing/lost_island/pos_4/check
