...
#select entity
execute if score temp_damage CAL matches 1.. as @e[distance=..5,team=hostile,scores={GAMELEVEL=0..},predicate=!att2_pre:unhurt,tag=!TEMP_ATK,limit=1] at @s on attacker if entity @s[advancements={att2_test:enchantment/groundslam=true}] run tag @e[team=hostile,scores={GAMELEVEL=0..},distance=..0,limit=1] add TEMP_ATK
#effect go
