...
execute unless score remove_progress ENCHANTMENT matches 70.. run return fail

playsound minecraft:block.conduit.deactivate player @a ~ ~ ~ 2 2
particle dust_color_transition{from_color:[1.0,0.0,0.0],scale:1.5,to_color:[0.0,0.0,1.0]} ~ ~ ~ 0.4 2 0.4 1 70
# reset nomarl rotation
tag @e[type=interaction,tag=INTERACT] add ENCHANTMENT
scoreboard players reset remove_progress ENCHANTMENT