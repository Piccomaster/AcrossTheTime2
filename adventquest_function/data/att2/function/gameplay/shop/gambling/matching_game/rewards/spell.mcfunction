...
execute if score SpellId MatchingGame matches 1 if predicate {condition:any_of,terms:[{condition:"minecraft:entity_scores",entity:this,scores:{SPELL1_CAP:{min:10}}},{condition:"minecraft:entity_properties",entity:this,predicate:{type_specific:{type:player,advancements:{"att2:dahal/spell1_loot":false}}}}]} run scoreboard players set Spell_Count MatchingGame 500
