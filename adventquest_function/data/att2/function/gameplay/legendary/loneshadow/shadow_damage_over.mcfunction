##################################################
#Made by Adventquest                             #
#Apply the effect of Lone Shadow       	         #
##################################################

#get player score
function att2:gameplay/score/player
#damage cal
scoreboard players operation @s SHADOW_TOTAL *= 10 CAL
#result damage
scoreboard players operation #damage CAL = @s SHADOW_TOTAL
#damage
execute as @e[distance=..10,scores={GAMELEVEL=0..},team=hostile,type=!bat] at @s run function att2:gameplay/legendary/loneshadow/damage

particle minecraft:warped_spore ~ ~1 ~ 1 1 1 0 100 normal
playsound minecraft:entity.warden.sonic_boom ambient @a ~ ~ ~ 1 1
##stop shadow_buff
scoreboard players set @s SHADOW_BUFF 0
scoreboard players set @s SHADOW_TOTAL 0
#effect
effect clear @s minecraft:darkness
effect clear @s minecraft:invisibility
tag @s remove ATTACK