...
#catch entity
execute if score TEST CAL matches 1 as @e[distance=..5,team=hostile,scores={GAMELEVEL=0..},tag=!ICED,tag=!ATK_FROZE,nbt={HurtTime:10s},limit=1] at @s on attacker if entity @s[advancements={att2_test:legendary/fenrir/attack_trigger=true}] as @e[distance=..0,limit=1,team=hostile,scores={GAMELEVEL=0..}] at @s run function att2:gameplay/legendary/fenrir/normal_atk/effect
#reset
