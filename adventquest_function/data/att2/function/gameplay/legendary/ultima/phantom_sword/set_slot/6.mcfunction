...
#say 槽位6
##append slot 1
data modify storage att2:ultima slot append value {slot:6,count:0}
##update slot data
...
##sync score
scoreboard players set @e[distance=..10,type=item_display,tag=New,tag=UltimaPhantom] ULTIMA_PS_COUNT 6
scoreboard players set @e[distance=..10,type=armor_stand,tag=New,tag=UltimaPhantom] ULTIMA_PS_COUNT 6
