...
#cal jo
scoreboard players operation #return_jo_count CAL = #jo RUNECOUNT

#cal max
scoreboard players operation #obtain_arrow_count CAL < #jo RUNECOUNT
scoreboard players operation #return_jo_count CAL -= #obtain_arrow_count CAL
...
##return jo
execute store result storage att2:score count int 1 run scoreboard players get #return_jo_count CAL
execute if score #return_jo_count CAL matches 1.. run loot spawn -5029 91 -4958 loot {"pools":[{"rolls":1,"entries":[{"type":"minecraft:item","name":"minecraft:glowstone_dust","functions":[{"function":"minecraft:set_components","components":{"minecraft:custom_name":{"translate":"item.rune.name.4"},"minecraft:lore":[{"translate":"item.rune.material"},{"translate":"item.rune.rarity.c"},{"translate":"item.rune.lvl.9"}],"max_stack_size":99,"minecraft:custom_data":{"EquipmentType":"rune","Rarity":"spe","Runelvl":4b},"minecraft:custom_model_data":{"floats":[10000005]},"minecraft:tooltip_style":"minecraft:rarity/rune/rune_low"}},{function:set_count,count:{type:"storage",storage:"att2:score",path:"count"}}]}]}]}


##summon poisoned_arrow_1
scoreboard players operation #obtain_arrow_count CAL *= 4 CAL
