...
execute at @s as @e[type=minecraft:iron_golem,tag=!NewInvo,scores={OWNER=1..}] unless score @s SUMMON_TIMER matches 1.. if score @s OWNER = @a[distance=..0,limit=1] NUMEROJOUEUR unless entity @e[type=minecraft:iron_golem,tag=!NewInvo,scores={OWNER=1..},distance=..15] run function att2:gameplay/dahal/action/spell26/follow_owner
