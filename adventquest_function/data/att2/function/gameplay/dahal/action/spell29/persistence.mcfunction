...
##store
execute store result storage att2:score speed double 0.15 run scoreboard players get #speed CAL
##motion
...
execute at @s positioned ~-1 ~-0.5 ~-1 unless entity @n[dx=2,dy=1,dz=2,scores={GAMELEVEL=0..},team=hostile,type=!bat,tag=!MIMIC,tag=!Invulnerable] run return fail


##damage
function att2:gameplay/dahal/action/spell29/damage_cal
##range damage effect
execute at @s positioned ~-1 ~-0.5 ~-1 as @e[dx=2,dy=1,dz=2,scores={GAMELEVEL=0..},team=hostile,type=!bat] at @s run function att2:gameplay/dahal/action/spell29/damage with storage att2:score
