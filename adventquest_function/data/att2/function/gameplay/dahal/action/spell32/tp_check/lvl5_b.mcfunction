...
execute at @s as @e[type=armor_stand,tag=SPELL32_SET_B,distance=81..160] if score @s OWNER = @a[distance=..0,limit=1] NUMEROJOUEUR run scoreboard players set DISTANCE CAL 2

execute at @s as @e[type=armor_stand,tag=SPELL32_SET_B,distance=161..200] if score @s OWNER = @a[distance=..0,limit=1] NUMEROJOUEUR run scoreboard players set DISTANCE CAL 3
#get esc score
