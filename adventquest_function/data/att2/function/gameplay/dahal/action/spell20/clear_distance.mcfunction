...
##Clear box miner
#execute as @s[scores={Stock_Open=1..}] as @n[distance=1.399..1.41,type=chest_minecart,tag=Stock,predicate=att2_pre:score/owner] run tellraw @a ["我活着",{score:{name:"@s",objective:"OWNER"}}]
execute as @s[scores={Stock_Open=1..}] unless entity @n[distance=1.399..1.41,type=chest_minecart,tag=Stock,predicate=att2_pre:score/owner] run function att2:gameplay/dahal/action/spell20/return/trigger
