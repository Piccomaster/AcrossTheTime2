...
execute as @s[scores={SPELL20_SLCT=1..,DAHAL=40..}] if entity @e[type=#minecraft:minecart,tag=other_minecart,distance=..5] at @s run scoreboard players set TEST CAL 1
execute as @s[scores={SPELL20_SLCT=1..,DAHAL=40..}] if entity @e[team=hostile,scores={GAMELEVEL=0..},distance=..5,limit=1] run scoreboard players set TEST CAL 2
## have minecarts near
...
##
execute as @s[scores={DAHAL=40..}] at @s anchored eyes positioned ^ ^-0.5 ^1 run function att2:gameplay/dahal/action/spell20/summon_trigger with storage att2:spell20