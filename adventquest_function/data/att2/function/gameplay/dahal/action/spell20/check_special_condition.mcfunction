...
execute if entity @e[type=#minecraft:minecart,tag=other_minecart,distance=..5] run scoreboard players set TEST CAL 1
...
## no stock
execute if score TEST CAL matches 1.. run function att2:gameplay/dahal/action/spell20/resummon
#resummon
...
execute if score TEST CAL matches 0 run scoreboard players set @s[scores={SPELL20_STOCK=1}] SPELL20_STOCK 0
#reset score
