...
execute if score level DIFFICULTY matches 1.. run scoreboard players add maxDG SPDG 30
execute if score level DIFFICULTY matches 2.. run scoreboard players add maxDG SPDG 40
#max-min
scoreboard players operation maxDG SPDG -= minDG SPDG
#cal  (SLCT_LVL-1)
scoreboard players operation slctlvl SPDG = @s SPELL1_SLCT
...
scoreboard players operation enhancementBONUS SPDG = BonusPowerSpell1 ENHANCEMENT
...
##critical detection
function att2:gameplay/dahal/action/critical/spell1_detection
#storage spell damage
execute store result storage att2:sp_dmg SP1 int 1 run scoreboard players get finalDG SPDG
#reset
#scoreboard players reset slctlvl SPDG
#scoreboard players reset finalDG SPDG

##set spell icon
data modify storage att2:enemy_health spell_icon set value "item/custom/spell_book/spell1"