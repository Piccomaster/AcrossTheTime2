...
execute anchored eyes at @s positioned ^ ^ ^0.4 run summon minecraft:snowball ~ ~ ~ {Tags:["FireballSpell","New"],Motion:[0,0,0],NoGravity:true,Silent:true,Item:{id:"minecraft:snowball",components:{item_model:nothing}},damage:-100,pickup:1,SoundEvent:"minecraft:intentionally_empty",life:1199,Passengers:[{id:"minecraft:fireball",Tags:["FireballSpell","display","New"],ExplosionPower:0b}]}

data modify entity @n[tag=New,type=snowball,tag=FireballSpell] Rotation set from entity @s Rotation
...
execute as @e[distance=..5,tag=New,type=snowball,tag=FireballSpell] run data modify entity @s Motion set from entity 00000001-0000-006f-0000-00010000006f Pos
#set owner
data modify entity @n[tag=New,type=snowball,tag=FireballSpell] Owner set from entity @s UUID
#back
...
#reset
tag @e[tag=New,type=snowball,tag=FireballSpell,distance=..10] remove New
