#################################################################
#Made by Adventquest											#
#Initialize boss felroth    									#
#################################################################

##pool1_7
scoreboard players reset pool1_7_t BOSS_TIME
scoreboard players reset pool1_7_s BOSS_TIME
scoreboard players reset pool1_7_m BOSS_TIME

scoreboard players set pool1_7_rt BOSS_TIME 7777777
scoreboard players set pool1_7_rs BOSS_TIME 7777777
scoreboard players set pool1_7_rm BOSS_TIME 7777777
