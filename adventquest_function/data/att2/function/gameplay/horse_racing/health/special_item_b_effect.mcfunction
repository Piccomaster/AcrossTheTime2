...
scoreboard players operation #health_add CAL *= -1 CAL
##text tip
execute on passengers if entity @s[tag=HorseRace] run tellraw @a[tag=HorseRace] [{translate:att2.dailyquest.horse_racing.special_item_b_effect.health_reduce,with:[{selector:"@p[predicate=att2_pre:score/player]",color:"dark_red"},{selector:"@s",color:"dark_red"},{score:{name:"#health_add",objective:CAL},color:"red"}]}]

##sound
playsound entity.villager.hurt master @p[predicate=att2_pre:score/player] ~ ~ ~ 150 1
##particle
execute on passengers if entity @s[tag=HorseRace] at @s run particle raid_omen ~ ~ ~ 1 1 1 1 10 normal
