...
##text tip
execute if score #health_add CAL matches ..-1 on passengers if entity @s[tag=HorseRace] run tellraw @a[tag=HorseRace] [{translate:att2.dailyquest.horse_racing.random_flaming_hoop.fail,with:[{selector:"@s",color:"dark_red"},{score:{name:"#health_add",objective:CAL},color:"red"},{score:{name:"#vitality_add",objective:CAL},color:"green"}]}]
