...
##text tip
execute if score #health_add CAL matches 1.. on passengers if entity @s[tag=HorseRace] run tellraw @a[tag=HorseRace] [{translate:att2.dailyquest.horse_racing.random_trap.health_add,with:[{selector:"@s",color:"dark_red"},{score:{name:"#health_add",objective:CAL},color:"green"}]}]
execute if score #health_add CAL matches ..-1 on passengers if entity @s[tag=HorseRace] run tellraw @a[tag=HorseRace] [{translate:att2.dailyquest.horse_racing.random_trap.health_reduce,with:[{selector:"@s",color:"dark_red"},{score:{name:"#health_add",objective:CAL},color:"red"}]}]
...
execute if score #health_add CAL matches ..-1 on passengers if entity @s[type=player] run playsound entity.villager.hurt master @s ~ ~ ~ 150 1
##particle
