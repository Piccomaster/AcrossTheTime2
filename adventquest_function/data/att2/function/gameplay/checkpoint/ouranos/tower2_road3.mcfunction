...
##show count
execute unless score Ouranos_road CHECKPOINT matches 20 positioned 7302 141 6518 run tellraw @a[distance=..40] [{text:"~",color:dark_red},{translate:att2.advancement.journey.title.checkpoint_ouranos_road,color:red},{text:":",color:gray},{score:{name:"Ouranos_road",objective:"CHECKPOINT"},color:red},{text:"/",color:gray},{text:"20",color:dark_red},{text:"~",color:dark_red}]

execute unless score Ouranos_road6 CHECKPOINT matches 1 positioned 7302 141 6518 run tellraw @a[distance=..40] [{translate:att2.checkpoint.trigger.new,color:green}]
