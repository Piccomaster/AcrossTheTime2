...
##show count
execute unless score Kert CHECKPOINT matches 10 positioned -5756 75 -4795 run tellraw @a[distance=..40] [{text:"~",color:dark_red},{translate:att2.advancement.journey.title.checkpoint_kert,color:red},{text:":",color:gray},{score:{name:"Kert",objective:"CHECKPOINT"},color:red},{text:"/",color:gray},{text:"10",color:dark_red},{text:"~",color:dark_red}]

execute unless score Kert1 CHECKPOINT matches 1 positioned -5756 75 -4795 run tellraw @a[distance=..40] [{translate:att2.checkpoint.trigger.new,color:green}]
