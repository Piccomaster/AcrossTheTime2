...
execute in minecraft:the_end run spawnpoint @s -1239 24 -536

## This command ensures that when a player manually triggers a respawn point, nearby players' respawn points are also reset.
execute in minecraft:the_end positioned -1239 24 -536 as @a[distance=..40] run function att2:gameplay/checkpoint/effect
execute in minecraft:the_end positioned -1239 24 -536 as @a[distance=..40] run spawnpoint @s -1239 24 -536
...
##show count
execute unless score Billgart_dungeon CHECKPOINT matches 39 positioned -1239 24 -536 run tellraw @a[distance=..40] [{text:"~",color:dark_red},{translate:att2.advancement.journey.title.checkpoint_billgart_dungeon,color:red},{text:":",color:gray},{score:{name:"Billgart_dungeon",objective:"CHECKPOINT"},color:red},{text:"/",color:gray},{text:"39",color:dark_red},{text:"~",color:dark_red}]

execute unless score Billgart_dungeon1 CHECKPOINT matches 1 positioned -1239 24 -536 run tellraw @a[distance=..40] [{translate:att2.checkpoint.trigger.new,color:green}]
