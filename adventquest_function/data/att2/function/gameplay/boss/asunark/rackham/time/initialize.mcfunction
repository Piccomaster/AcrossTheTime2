#################################################################
#Made by Adventquest											#
#Initialize boss felroth    									#
#################################################################

##rackham

scoreboard players reset rackham_t BOSS_TIME
scoreboard players reset rackham_s BOSS_TIME
scoreboard players reset rackham_m BOSS_TIME

scoreboard players set newrecord BOSS_TIME 0
scoreboard players set rackham_rt BOSS_TIME 7777777
scoreboard players set rackham_rs BOSS_TIME 7777777
scoreboard players set rackham_rm BOSS_TIME 7777777

scoreboard players set 50 BOSS_TIME 50