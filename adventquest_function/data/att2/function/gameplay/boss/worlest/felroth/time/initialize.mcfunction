#################################################################
#Made by Adventquest											#
#Initialize boss felroth    									#
#################################################################

##felroth

scoreboard players reset felroth_t BOSS_TIME
scoreboard players reset felroth_s BOSS_TIME
scoreboard players reset felroth_m BOSS_TIME

scoreboard players set newrecord BOSS_TIME 0
scoreboard players set felroth_rt BOSS_TIME 7777777
scoreboard players set felroth_rs BOSS_TIME 7777777
scoreboard players set felroth_rm BOSS_TIME 7777777

scoreboard players set 50 BOSS_TIME 50