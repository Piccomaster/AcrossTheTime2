...
tag @s add TEMP
#get victim entity
execute as @e[type=!player,nbt={HurtTime:10s}] at @s on attacker if entity @s[advancements={att2_test:enchantment/headhunter=true}] run tag @e[type=!player,distance=..0,limit=1] add TEMP_ATK
#get arrow
