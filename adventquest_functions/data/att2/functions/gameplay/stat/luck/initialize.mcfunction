##################################################
#Made by Adventquest                             #
#Initialize luck                                 #
##################################################

scoreboard objectives add LUC dummy
scoreboard players set @s LUC 0

scoreboard objectives add LUC_DATA dummy
scoreboard players set @s LUC_DATA 0

scoreboard objectives add LUC_TOT dummy
scoreboard players set @s LUC_TOT 0

scoreboard objectives add LUC_BASE dummy
scoreboard players set @s LUC_BASE 0

scoreboard objectives add LUC_EQ dummy
scoreboard players set @s LUC_EQ 0

scoreboard objectives add LUC_SP dummy
scoreboard players set @s LUC_SP 0

scoreboard objectives add LUC_PO dummy
scoreboard players set @s LUC_PO 0

scoreboard objectives add LUC_EH dummy
scoreboard players set @s LUC_EH 0

scoreboard objectives add LUC_EXT dummy
scoreboard players set @s LUC_EXT 0

scoreboard objectives add TIMER_LUC_SP dummy
scoreboard objectives add TIMER_LUC_EQ dummy
scoreboard objectives add TIMER_LUC_EXT dummy
scoreboard objectives add TIMER_LUC_PO dummy
scoreboard objectives add TIMER_LUC_EH dummy

scoreboard objectives add LUC_UPGRADE_REQ dummy
scoreboard players set @s LUC_UPGRADE_REQ 3